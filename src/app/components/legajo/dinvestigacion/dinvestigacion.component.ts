...
@Component({
  selector: 'app-dinvestigacion',
  templateUrl: './dinvestigacion.component.html',
  styleUrls: ['./dinvestigacion.component.sass'],
  providers: [
    { provide: DateAdapter, useClass: AppDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS },
  ],
})
export class DinvestigacionComponent implements OnInit {
  @ViewChild('drawer') sideNav: any
  @ViewChild('stepper') stepper: any
  @ViewChild('stepper2') stepper2: any
  @ViewChild('stepper3') stepper3: any
  modulo: string = 'LEGAJOS'
  title: string = 'Investigaciones'
  codigoMod: number = 1
  codigoArea: number = 0
  route: string = 'dinvestigacion'
  bValidar: number = 0
  ...
  ngOnInit(): void {
    ...
    this.lstserv.listar_grupo_investigacion(this.segserv.usuarioreg.cPerCodigo)
  }
    ...
    if (this.segserv.usuarioreg.nRol == 1) {
      this.lstserv
        ...
        .then((data) => {
          ...
          for (
            let index = 0;
            index < this.lstserv.lTareasPermiso.length;
            index++
          ) {
  ...
  nuevo_GrupoInvestigacion($Leg_Grup_Inv_Sem: Leg_Grup_Inv_Sem) {
    this.mdlserv.nuevoModalGrupoInvestigacion(
      'Líder de grupo de investigación o semilleros',
      this.regLegDatosGenerales.nLegDatCodigo,
      $Leg_Grup_Inv_Sem,
      this.segserv.usuarioreg.cPerCodigo,
    )
  ...
  eliminar_grup_inv($obg: Leg_Grup_Inv_Sem, bvalel: boolean) {
    Swal.fire({
      title: 'Registro de Legajos',
      text: bvalel
        ? '¿Está seguro de eliminar registro?'
