...
@Component({
  selector: 'app-dcapacitaciones',
  templateUrl: './dcapacitaciones.component.html',
  styleUrls: ['./dcapacitaciones.component.sass'],
  providers: [
    { provide: DateAdapter, useClass: AppDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS }
  ]
})
export class DcapacitacionesComponent implements OnInit {
  @ViewChild('drawer') sideNav: any;
  @ViewChild('stepper') stepper: any;
  @ViewChild('stepper2') stepper2: any;
  @ViewChild('stepper3') stepper3: any;
  modulo: string = 'LEGAJOS';
  title: string = 'Capacitaciones';
  codigoMod: number = 1;
  codigoArea: number = 0;
  route: string = 'dcapacitaciones'
  bValidar: number = 0;
  ...
  constructor(
    ...
    if (this.segserv.conexionactiva()) {
      this.spinner.show()
      // Swal.fire("Declaración Jurada", "Todos los datos a registrar en los siguientes formularios ................", "success");
      ...
      this.ctrlserv._banregister = this.segserv.usuarioreg.nRol == 1 ? false : true
        ...
        this.lstserv.listado('legajoaux/', datos.cPerCodigo).then((data) => {
          ...
          if (JSON.stringify(data) != '[]') {
            if (data.nLegDatCodigo == 0 || data.nLegDatCodigo == null) {
              Swal.fire("Control de Legajos", "No se ha encontrado datos generales. Solicite que el colaborador actualice sus datos.", "info")
            } else {
      ...
      case this.configserv.nTareaValidar:
        ...
        this.lstserv.listado('legajoaux/', datos.cPerCodigo).then((data) => {
          ...
          if (JSON.stringify(data) != '[]') {
            ...
            if (data.nLegDatCodigo == 0 || data.nLegDatCodigo == null) {
              this.spinner.hide()
              Swal.fire("Control de Legajos", "No se ha encontrado datos datos generales. Solicite que el colaborador actualice sus datos.", "info")
            } else {
              this.bValidar = 12;
  ...
  listadocolaborador() {
    ...
    console.log('dddddd')
    this.spinner.show()
    this.lstserv.listado('listausuarios', '/' + this.configserv.nTipoColaborador+'/'+'0').then((data) => {
      ...
      if (this.lstserv.listColaboradores.length != this.dataglobal.length) {
        ...
        this.pageService.actualizaTabla2(this.lstserv.listColaboradores, this.pageEvent);
      }
      this.spinner.hide()
    })
  ...
  cargardata() {
    this.filteredOptions = this.myControl.valueChanges
      .pipe(
        ...
        map(cPerNombre => cPerNombre ? this._filter(cPerNombre) : this.lstserv.lColegio.slice())
      );
  }
  ...
  private _filter(name: string): Persona[] {
    ...
    return this.lstserv.lColegio.filter(option => option.cPerNombre.toLowerCase().includes(filterValue)).slice(0, 100);
  }
  ...
  handleModelChange(e: any, tipo: number) {
    if (e == '') {
      switch (tipo) {
        case 1: this.regLegDatosGenerales.cLegDatColegioProf = ''; break;
      }
  ...
  listar_tipodocidentidad() {
    this.lstserv.listado('interface', '/' + this.configserv.nIntTipoDocIdent).then((data) => {
      let ltipodoc: Interface[] = data;
      this.lstserv.lTipoDocIdentidad = ltipodoc.filter(x => x.nIntCodigo != 0)
    })
  }

  listar_gradoacademico() {
    this.lstserv.listado('interface', '/' + this.configserv.nIntGradoAcad).then((data) => {
  ...
  listar_tipodomicilio() {
    this.lstserv.listado('constante', '/' + this.configserv.nConTipoDomicilio).then((data) => {
  ...
  listar_tipozona() {
    this.lstserv.listado('constante', '/' + this.configserv.nConTipoZona).then((data) => {
  ...
  listar_condicion() {
    this.lstserv.listado('constante', '/' + this.configserv.nConCondicion).then((data) => {
      this.lstserv.lCondicionColeg = data
    })
  }

  listar_sexo() {
    this.lstserv.listado('constante', '/' + this.configserv.nConSexo).then((data) => {
  ...
  listar_estadocivil() {
    this.lstserv.listado('constante', '/' + this.configserv.nConEstadoCivil).then((data) => {
  ...
  listar_universidad() {
    this.lstserv.listado('persona', '/' + this.configserv.nPerUniversidad).then((data) => {
  ...
  listar_colegio() {
    this.lstserv.listado('persona', '/' + this.configserv.nPerColegio).then((data) => {
  ...
  listar_tareaspermiso() {
    this.segserv.obtenerdatosusuario()
    if (this.segserv.usuarioreg.nRol == 1) {
      this.lstserv.listado('tareamodulo', '/' + this.segserv.usuarioreg.cPerCodigo + '/' + this.codigoMod).then((data) => {
  ...
  listar_ubigeo() {
    this.ctrlserv._banregister = true
    this.spinner.show()
    this.lstserv.listado('interface', '/' + this.configserv.nIntUbigeo).then((data) => {
      let lubigeo: Interface[] = data;
      this.lstserv.lubigeo = lubigeo.filter(x => x.nIntCodigo != 0)
      this.lstserv.lnacionalidad = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 3)
      this.lstserv.lpaisNac = this.lstserv.lnacionalidad
      this.lstserv.filterpais = this.lstserv.lpaisNac.slice()
      this.lstserv.ldepartamento = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 2)
      this.segserv.obtenerdatosusuario()
      this.editar_cv(this.segserv.usuarioreg)
  ...
  ubigeoOnChange($obj: any, $tipo: number) {
    if ($obj == null) {
      ...
      switch ($tipo) {
        case 1:
          this.lstserv.lprovincia = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 4 && x.cIntJerarquia.trim().substring(0, 2) == $obj.cIntJerarquia.trim())
          this.lstserv.ldistrito = []
          break;
        case 2:
          this.lstserv.ldistrito = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 6 && x.cIntJerarquia.trim().substring(0, 4) == $obj.cIntJerarquia.trim())
          break;
        case 3:
        ...
        case 0:
          if ($obj.cIntJerarquia.trim() == 'PER') {
            this.lstserv.ldepartamentoNac = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 2)
            this.lstserv.lprovinciaNac = []
            this.lstserv.ldistritoNac = []
            this.regLegDatosGenerales.nLetDatNacimiento = 0
            this.regLegDatosGenerales.nClaseNacimiento = 0
          ...
          break;
        case 4:
          this.lstserv.lprovinciaNac = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 4 && x.cIntJerarquia.trim().substring(0, 2) == $obj.cIntJerarquia.trim())
          this.lstserv.ldistritoNac = []
          break;
        case 5:
          this.lstserv.ldistritoNac = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim().length == 6 && x.cIntJerarquia.trim().substring(0, 4) == $obj.cIntJerarquia.trim())
          break;
        case 6:
          this.regLegDatosGenerales.nLetDatNacimiento = $obj.nIntCodigo
          this.regLegDatosGenerales.nClaseNacimiento = $obj.nIntClase
  ...
  guardarDJ() {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      Swal.fire("Control de Legajos", "No se ha encontrado datos generales, guarde primero los datos generales.", "info")
    } else {

      var formData = new FormData();
      let fecha: any
      formData.append('nLegDjcodigo', this.lstserv.LegDeclaracionJurada.nLegDjcodigo.toString() ?? '0')
      formData.append('nLegDjdatCodigo', this.regLegDatosGenerales.nLegDatCodigo.toString().toString() ?? '0')
      ...
      if (this.lstserv.LegDeclaracionJurada.cLegDjanexo2 != this.lstserv.LegDeclaracionJurada.cFileDjanexo2) {
        formData.append('cFileDjanexo2', this.lstserv.LegDeclaracionJurada.cLegDjanexo2)
        formData.append('cLegDjanexo2', '')
      } else {
        formData.append('cLegDjanexo2', this.lstserv.LegDeclaracionJurada.cLegDjanexo2)
      }
      if (this.lstserv.LegDeclaracionJurada.cLegDjanexo6 != this.lstserv.LegDeclaracionJurada.cFileDjanexo6) {
        formData.append('cFileDjanexo6', this.lstserv.LegDeclaracionJurada.cLegDjanexo6)
        formData.append('cLegDjanexo6', '')
      } else {
        formData.append('cLegDjanexo6', this.lstserv.LegDeclaracionJurada.cLegDjanexo6)
      }
      if (this.lstserv.LegDeclaracionJurada.cLegDjanexo7 != this.lstserv.LegDeclaracionJurada.cFileDjanexo7) {
        formData.append('cFileDjanexo7', this.lstserv.LegDeclaracionJurada.cLegDjanexo7)
        formData.append('cLegDjanexo7', '')
      } else {
        formData.append('cLegDjanexo7', this.lstserv.LegDeclaracionJurada.cLegDjanexo7)
      }

      formData.append('bLegDjestado', 'true')

      this.regserv.registroFile('Registro de Legajos', 'declaracionjurada/' + this.regLegDatosGenerales.nLegDatCodigo, formData).then((data) => {
  ...
  nuevo_gradotitulo($formacion: LegGradoTitulo) {
    this.mdlserv.nuevoModalGradoTitulo('Grados y títulos', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
  }

  nuevo_experienciadoc($obg: LegDocenciaUniv) {
    this.mdlserv.nuevoModalExperienciaDoc('Docencia universitaria', this.regLegDatosGenerales.nLegDatCodigo, $obg);
  }

  nuevo_categoriadoc($obg: LegCategoriaDocente) {
    this.mdlserv.nuevoModalCategoriaDOc('Categoría docente', this.regLegDatosGenerales.nLegDatCodigo, $obg);
  }

  nuevo_dedicaciondoc($obg: LegRegimenDedicacion) {
    this.mdlserv.nuevoModalRegimenDed('Régimen dedicación docente', this.regLegDatosGenerales.nLegDatCodigo, $obg);
  }

  nuevo_experiencianodoc($obg: LegProfesNoDocente) {
    this.segserv.obtenerdatosusuario()
    this.mdlserv.nuevoModalProfsNoDocenete(this.ban_administrativo ? 'Experiencia profesional' : 'Experiencia profesional no docente', this.regLegDatosGenerales.nLegDatCodigo, $obg, !this.ban_administrativo);

  }

  nuevo_idiomasofimatica($obg: LegIdiomaOfimatica, tipo: boolean) {
    this.mdlserv.nuevoModalIdiomaOfimatica((tipo ? 'Dominio de ofimática' : 'Dominio de idiomas'), this.regLegDatosGenerales.nLegDatCodigo, tipo, $obg);
  }

  nuevo_investigador($obg: LegInvestigador) {
    this.mdlserv.nuevoModalIvestigador('Docente investigador', this.regLegDatosGenerales.nLegDatCodigo, $obg, this.segserv.usuarioreg.cPerCodigo);
  }

  nuevo_asejurtesis($obg: LegTesisAseJur) {
    this.mdlserv.nuevoModalTesisAsesJur('Asesoría y jurado de tesis', this.regLegDatosGenerales.nLegDatCodigo, $obg, this.segserv.usuarioreg.cPerCodigo);
  }

  nuevo_produccionciencia($obg: LegProduccionCiencia) {
    this.mdlserv.nuevoModalProduccionCiencia('Produccción científica, lectiva y de investigación', this.regLegDatosGenerales.nLegDatCodigo, $obg, this.segserv.usuarioreg.cPerCodigo);
  }

  nuevo_participacion($obg: LegParticipacionCongSem) {
    this.mdlserv.nuevoModalParticipacion('Participación eventos', this.regLegDatosGenerales.nLegDatCodigo, $obg, this.segserv.usuarioreg.cPerCodigo);
  }

  nuevo_cargadmuniv($obg: LegAdminitrativaCarga) {
    this.mdlserv.nuevoModalCargaAdmin('Carga administrativa universitaria', this.regLegDatosGenerales.nLegDatCodigo, $obg);
  }

  nuevo_reconocimiento($obg: LegReconocimiento) {
    this.mdlserv.nuevoModalReconocimiento('Reconocimiento de otras instituciones', this.regLegDatosGenerales.nLegDatCodigo, $obg);
  }

  nuevo_capacitacion($obg: LegCapacitaciones) {
    this.segserv.obtenerdatosusuario()
    this.mdlserv.nuevoModalCapacitaciones('Capacitaciones', this.regLegDatosGenerales.nLegDatCodigo, $obg, !this.ban_administrativo, this.segserv.usuarioreg.cPerCodigo);
  }

  nuevo_proyeccionsocial($obg: LegProyeccionSocial) {
    this.mdlserv.nuevoModalProyeccionSoc('Proyección social', this.regLegDatosGenerales.nLegDatCodigo, $obg);
  }
  ...
  nuevo_documentacionint($formacion: LegDocumentacionInterna) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalDocumentacionInterna('Documentación interna', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
    }
  }
  ...
  nuevo_capacitacionint($formacion: LegCapacitacionInterna) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalCapacitacionInterna('Capacitación interna', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
    }
  }
  ...
  nuevo_contratoac($formacion: LegContrato) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalContrato('Contrato', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
    }
  }
  ...
  nuevo_resolucionac($formacion: LegResolucion) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalResolucion('Resolución', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
    }
  }
  ...
  nuevo_evaluaciondesempac($formacion: LegEvaluacionDesemp) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalEvaluacionDesem('Evauación de desempeño', this.regLegDatosGenerales.nLegDatCodigo, $formacion, !this.ban_administrativo);
    }
  }
  ...
  nuevo_seleccionac($formacion: LegSeleccion) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalSeleccion('Selección', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
    }
  }
  ...
  nuevo_ordinarizacionac($formacion: LegOrdinarizacion) {
    if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
      ...
      this.mdlserv.nuevoModalOrdinarizacion('Ordinarización', this.regLegDatosGenerales.nLegDatCodigo, $formacion);
    }
  }

  nuevo_capacitacioninterna($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalCapacitacionInterna('Capacitación interna', data.nLegDatCodigo, this.clmdserv.empty_capacitacioninterna());
        } else {
          this.limpiar_controles()
          this.spinner.hide()
      ...
      Swal.fire("Control de Legajos", "No se ha encontrado datos generales.", "info")
    }
  }
  ...
  nuevo_contrato($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalContrato('Registro contrato', data.nLegDatCodigo, this.clmdserv.empty_contratos());
        } else {
          this.limpiar_controles()
          this.spinner.hide()
          Swal.fire("Control de Legajos", "No se ha encontrado datos, proceda a completar los datos solicitados.", "info")
  ...
  nuevo_resolucion($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalResolucion('Registro resolución', data.nLegDatCodigo, this.clmdserv.empty_resoluciones());
        } else {
          this.limpiar_controles()
          this.spinner.hide()
          Swal.fire("Control de Legajos", "No se ha encontrado datos, proceda a completar los datos solicitados.", "info")
  ...
  nuevo_evaluaciondsemp($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalEvaluacionDesem('Registro Evaluación de Desempeño', data.nLegDatCodigo, this.clmdserv.empty_evaluaciondesemp(), $objaux.nTipo == 2 ? false : true);
        } else {
          this.limpiar_controles()
          this.spinner.hide()
          Swal.fire("Control de Legajos", "No se ha encontrado datos, proceda a completar los datos solicitados.", "info")
  ...
  nuevo_seleccion($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalSeleccion('Registro selección', data.nLegDatCodigo, this.clmdserv.empty_seleccion());
        } else {
          this.limpiar_controles()
          this.spinner.hide()
          Swal.fire("Control de Legajos", "No se ha encontrado datos, proceda a completar los datos solicitados.", "info")
  ...
  nuevo_ordinarizacion($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalOrdinarizacion('Registro ordinarización', data.nLegDatCodigo, this.clmdserv.empty_ordinarizacion());
        } else {
          this.limpiar_controles()
          this.spinner.hide()
          Swal.fire("Control de Legajos", "No se ha encontrado datos, proceda a completar los datos solicitados.", "info")
  ...
  nuevo_documentacioninterna($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo/', $objaux.cPerCodigo).then((data) => {
        if (JSON.stringify(data) != '[]') {
          this.mdlserv.nuevoModalDocumentacionInterna('Registro Documentación Interna', data.nLegDatCodigo, this.clmdserv.empty_documentacioninterna());
        } else {
          this.limpiar_controles()
          this.spinner.hide()
          Swal.fire("Control de Legajos", "No se ha encontrado datos, proceda a completar los datos solicitados.", "info")
  ...
  editar_capacitacioninterna($obj: LegCapacitacionInterna) {
    this.mdlserv.nuevoModalCapacitacionInterna('Capacitación interna', $obj.nLegDatCodigo, $obj);
  }
  ...
  editar_evaluaciondesemp($obj: LegEvaluacionDesemp) {
    ...
    this.mdlserv.nuevoModalEvaluacionDesem('Evaluación de Desempeño', $obj.nLegEvalDatCodigo, $obj, this.ban_administrativo);
  }
  ...
  editar_ordinarizacion($obj: LegOrdinarizacion) {
    this.mdlserv.nuevoModalOrdinarizacion('Ordinrizacion', $obj.nLegOrdDatCodigo, $obj);
  }
  ...
  eliminar_validar_formacion($obj: LegGradoTitulo, bvalel: boolean) {
    Swal.fire({
      ...
      text: bvalel ? '¿Está seguro de eliminar registro?' : '¿Está seguro de validar registro?',
      icon: 'question',
      ...
      if (result.value) {
        if ($obj.nLegGraCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'gradotitulo/' + $obj.nLegGraCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegDocCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'docenciauniv/' + $obg.nLegDocCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegCatCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'categoriadocente/' + $obg.nLegCatCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegRegCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'regimendedicacion/' + $obg.nLegRegCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegProCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'experiencianodoc/' + $obg.nLegProCodigo, request).then(() => {
  ...
  eliminar_validar_idiomasofimatica($obg: LegIdiomaOfimatica, tipo: boolean, bvalel: boolean) {
    Swal.fire({
      title: "Registro de Legajos",
      text: bvalel ? '¿Está seguro de eliminar registro?' : '¿Está seguro de validar registro?',
      icon: 'question',
      ...
      if (result.value) {
        if ($obg.nLegIdOfCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'idiomaofimatica/' + $obg.nLegIdOfCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegInvCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'investigador/' + $obg.nLegInvCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegTesCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'tesisasejur/' + $obg.nLegTesCodigo, request).then(() => {
  ...
  eliminar_validar_produccionciencia($obg: LegProduccionCiencia, bvalel: boolean) {
    Swal.fire({
      title: "Registro de Legajos",
      text: bvalel ? '¿Está seguro de eliminar registro?' : '¿Está seguro de validar registro?',
      icon: 'question',
      ...
      if (result.value) {
        if ($obg.nLegProdCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'produccionciencia/' + $obg.nLegProdCodigo, request).then(() => {
  ...
  eliminar_validar_participacion($obg: LegParticipacionCongSem, bvalel: boolean) {
    Swal.fire({
      title: "Registro de Legajos",
      text: bvalel ? '¿Está seguro de eliminar registro?' : '¿Está seguro de validar registro?',
      icon: 'question',
      ...
      if (result.value) {
        if ($obg.nLegParCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'participacion/' + $obg.nLegParCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegAdmCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'cargaadmin/' + $obg.nLegAdmCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegRecCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'reconocimiento/' + $obg.nLegRecCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegCapCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'capacitacion/' + $obg.nLegCapCodigo, request).then(() => {
  ...
  eliminar_validar_proyeccionsocial($obg: LegProyeccionSocial, bvalel: boolean) {
    Swal.fire({
      title: "Registro de Legajos",
      text: bvalel ? '¿Está seguro de eliminar registro?' : '¿Está seguro de validar registro?',
      icon: 'question',
      ...
      if (result.value) {
        if ($obg.nLegProyCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'proyeccionsocial/' + $obg.nLegProyCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegCicodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'legcapacitacioninterna/' + $obg.nLegCicodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegConCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'contrato/' + $obg.nLegConCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegResCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'resolucion/' + $obg.nLegResCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegEvalCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'evaluaciondesemp/' + $obg.nLegEvalCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegSelCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'seleccion/' + $obg.nLegSelCodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegDicodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'documentacioninterna/' + $obg.nLegDicodigo, request).then(() => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        if ($obg.nLegOrdCodigo > 0) {
          ...
          this.regserv.actualizarcampo('Registro de Legajo', 'ordinarizacion/' + $obg.nLegOrdCodigo, request).then(() => {
  ...
  guardarcv() {
    try {
      ...
      this.regLegDatosGenerales.cLegDatColegioProf = (this.regLegDatosGenerales.cLegDatColegioProf == null ? '' : this.regLegDatosGenerales.cLegDatColegioProf)
      ...
      let fechaFin: any = this.configserv.datepipe.transform(new Date())
      ...
      if (this.regLegDatosGenerales.nLegDatCodigo == 0) {
        ...
        // }
        this.spinner.show()
        if (this.lstserv.LegDeclaracionJurada.cLegDjanexo2 != null && this.lstserv.LegDeclaracionJurada.cLegDjanexo6 != null && this.lstserv.LegDeclaracionJurada.cLegDjanexo7 == null) {
          formData.append('legDeclaracionJurada[0].nLegDjcodigo', this.lstserv.LegDeclaracionJurada.nLegDjcodigo.toString() ?? '0')
          formData.append('legDeclaracionJurada[0].cFileDjanexo2', this.lstserv.LegDeclaracionJurada.cLegDjanexo2)
          formData.append('legDeclaracionJurada[0].cFileDjanexo6', this.lstserv.LegDeclaracionJurada.cLegDjanexo6)
          formData.append('legDeclaracionJurada[0].cFileDjanexo7', this.lstserv.LegDeclaracionJurada.cLegDjanexo7)
        ...
        this.lstserv.lLegGradoTitulo.forEach((element: LegGradoTitulo, index) => {
          let fechaobt: any = this.configserv.datepipe.transform(element.dLegGraFecha, "yyyy-MM-dd")
          formData.append('legGradoTitulo[' + index + '].nLegGraCodigo', element.nLegGraCodigo.toString() ?? '0')
          formData.append('legGradoTitulo[' + index + '].nLegGraGradoAcad', element.nLegGraGradoAcad.toString() ?? '0')
          formData.append('legGradoTitulo[' + index + '].nClaseGradoAcad', element.nClaseGradoAcad.toString() ?? '0')
          formData.append('legGradoTitulo[' + index + '].cLegGraCarreraProf', element.cLegGraCarreraProf.toString() ?? '')
          formData.append('legGradoTitulo[' + index + '].cLegGraInstitucion', element.cLegGraInstitucion.toString() ?? '0')
          formData.append('legGradoTitulo[' + index + '].cLegGraOtraInst', element.cLegGraOtraInst.toString() ?? '0')
          formData.append('legGradoTitulo[' + index + '].nLegGraPais', element.nLegGraPais.toString() ?? '0')
          formData.append('legGradoTitulo[' + index + '].nClasePais', element.nClasePais.toString() ?? '0')
          ...
          formData.append('legGradoTitulo[' + index + '].dLegGraFecha', fechaobt)
          formData.append('legGradoTitulo[' + index + '].cFile', element.cLegGraArchivo)
          formData.append('legGradoTitulo[' + index + '].cLegGraArchivo', '')
          formData.append('legGradoTitulo[' + index + '].cLegGraValida', 'false')
          formData.append('legGradoTitulo[' + index + '].cLegGraEstado', 'true')
        });

        this.lstserv.lLegDocenciaUniv.forEach((element: LegDocenciaUniv, index) => {
          formData.append('legDocenciaUniv[' + index + '].nLegDocCodigo', element.nLegDocCodigo.toString() ?? '0')
          formData.append('legDocenciaUniv[' + index + '].nLegDocRegimen', element.nLegDocRegimen.toString() ?? '0')
          formData.append('legDocenciaUniv[' + index + '].nValorRegimen', element.nValorRegimen.toString() ?? '0')
          formData.append('legDocenciaUniv[' + index + '].nLegDocCategoria', element.nLegDocCategoria.toString() ?? '0')
          formData.append('legDocenciaUniv[' + index + '].nValorCategoria', element.nValorCategoria.toString() ?? '0')
          formData.append('legDocenciaUniv[' + index + '].cLegDocUniversidad', element.cLegDocUniversidad.toString() ?? '')
          formData.append('legDocenciaUniv[' + index + '].cLegDocOtraInst', element.cLegDocOtraInst.toString() ?? '')
          formData.append('legDocenciaUniv[' + index + '].cLegDocPais', element.cLegDocPais.toString() ?? '')
          fecha = this.configserv.datepipe.transform(element.dLegDocFechaInicio, "yyyy-MM-dd")
          formData.append('legDocenciaUniv[' + index + '].dLegDocFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegDocFechaFin, "yyyy-MM-dd")
          formData.append('legDocenciaUniv[' + index + '].dLegDocFechaFin', fecha)
          formData.append('legDocenciaUniv[' + index + '].cFile', element.cLegDocArchivo)
          formData.append('legDocenciaUniv[' + index + '].cLegGraArchivo', '')
          formData.append('legDocenciaUniv[' + index + '].cLegDocValida', 'false')
          formData.append('legDocenciaUniv[' + index + '].cLegDocEstado', 'true')
        });

        this.lstserv.lCategoriaDocente.forEach((element: LegCategoriaDocente, index) => {
          formData.append('legCategoriaDocente[' + index + '].nLegCatCodigo', element.nLegCatCodigo.toString() ?? '0')
          formData.append('legCategoriaDocente[' + index + '].cLegCatInstitucion', element.cLegCatInstitucion.toString() ?? '')
          formData.append('legCategoriaDocente[' + index + '].cLegCatOtraInst', element.cLegCatOtraInst.toString() ?? '')
          formData.append('legCategoriaDocente[' + index + '].cLegCatPais', element.cLegCatPais.toString() ?? '')
          formData.append('legCategoriaDocente[' + index + '].nLegCatCategoria', element.nLegCatCategoria.toString() ?? '0')
          formData.append('legCategoriaDocente[' + index + '].nValorCategoria', element.nValorCategoria.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegCatFechaInicio, "yyyy-MM-dd")
          formData.append('legCategoriaDocente[' + index + '].dLegCatFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegCatFechaFin, "yyyy-MM-dd")
          formData.append('legCategoriaDocente[' + index + '].dLegCatFechaFin', fecha)
          formData.append('legCategoriaDocente[' + index + '].cFile', element.cLegCatArchivo)
          formData.append('legCategoriaDocente[' + index + '].cLegGraArchivo', '')
          formData.append('legCategoriaDocente[' + index + '].cLegCatValida', 'false')
          formData.append('legCategoriaDocente[' + index + '].cLegCatEstado', 'true')
        });

        this.lstserv.lRegimenDedic.forEach((element: LegRegimenDedicacion, index) => {
          formData.append('legRegimenDedicacion[' + index + '].nLegRegCodigo', element.nLegRegCodigo.toString() ?? '0')
          formData.append('legRegimenDedicacion[' + index + '].cLegCatInstitucion', element.cLegCatInstitucion.toString() ?? '')
          formData.append('legRegimenDedicacion[' + index + '].cLegRegOtraInst', element.cLegRegOtraInst.toString() ?? '')
          formData.append('legRegimenDedicacion[' + index + '].cLegRegPais', element.cLegRegPais.toString() ?? '')
          formData.append('legRegimenDedicacion[' + index + '].nLegRegDedicacion', element.nLegRegDedicacion.toString() ?? '0')
          formData.append('legRegimenDedicacion[' + index + '].nValorDedicacion', element.nValorDedicacion.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegRegFechaInicio, "yyyy-MM-dd")
          formData.append('legRegimenDedicacion[' + index + '].dLegRegFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegRegFechaFin, "yyyy-MM-dd")
          formData.append('legRegimenDedicacion[' + index + '].dLegRegFechaFin', fecha)
          formData.append('legRegimenDedicacion[' + index + '].cFile', element.cLegRegArchivo)
          formData.append('legRegimenDedicacion[' + index + '].cLegRegArchivo', '')
          formData.append('legRegimenDedicacion[' + index + '].cLegRegValida', 'false')
          formData.append('legRegimenDedicacion[' + index + '].cLegRegEstado', 'true')
        });

        this.lstserv.lProfesNoDocente.forEach((element: LegProfesNoDocente, index) => {
          formData.append('legProfesNoDocente[' + index + '].nLegProCodigo', element.nLegProCodigo.toString() ?? '0')
          formData.append('legProfesNoDocente[' + index + '].cLegProInstitucion', element.cLegProInstitucion.toString() ?? '')
          formData.append('legProfesNoDocente[' + index + '].cLegProOtraInst', element.cLegProOtraInst.toString() ?? '')
          formData.append('legProfesNoDocente[' + index + '].cLegProPais', element.cLegProPais.toString() ?? '')
          formData.append('legProfesNoDocente[' + index + '].nLegProCargo', element.nLegProCargo.toString() ?? '0')
          formData.append('legProfesNoDocente[' + index + '].cLegProCargoProf', element.cLegProCargoProf.toString() ?? '')
          formData.append('legProfesNoDocente[' + index + '].nValorCargo', element.nValorCargo.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegProFechaInicio, "yyyy-MM-dd")
          formData.append('legProfesNoDocente[' + index + '].dLegProFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegProFechaFin, "yyyy-MM-dd")
          formData.append('legProfesNoDocente[' + index + '].dLegProFechaFin', fecha)
          formData.append('legProfesNoDocente[' + index + '].cFile', element.cLegProArchivo)
          formData.append('legProfesNoDocente[' + index + '].cLegProArchivo', '')
          formData.append('legProfesNoDocente[' + index + '].cLegProValida', 'false')
          formData.append('legProfesNoDocente[' + index + '].cLegProEstado', 'true')
        });

        this.lstserv.lidiomasofimatica.forEach((element: LegIdiomaOfimatica, index) => {
          formData.append('legIdiomaOfimatica[' + index + '].nLegIdOfCodigo', element.nLegIdOfCodigo.toString() ?? '0')
          formData.append('legIdiomaOfimatica[' + index + '].nLegIdOfCodigoDesc', element.nLegIdOfCodigoDesc.toString() ?? '')
          formData.append('legIdiomaOfimatica[' + index + '].nValorDesc', element.nValorDesc.toString() ?? '0')
          formData.append('legIdiomaOfimatica[' + index + '].nLegIdOfNivel', element.nLegIdOfNivel.toString() ?? '0')
          formData.append('legIdiomaOfimatica[' + index + '].nValorNivel', element.nValorNivel.toString() ?? '0')
          formData.append('legIdiomaOfimatica[' + index + '].cLegIdOfTipo', element.cLegIdOfTipo.toString() ?? '')
          fecha = this.configserv.datepipe.transform(element.dLegIdOfFecha, "yyyy-MM-dd")
          formData.append('legIdiomaOfimatica[' + index + '].dLegIdOfFecha', fecha)
          formData.append('legIdiomaOfimatica[' + index + '].cFile', element.cLegIdOfArchivo)
          formData.append('legIdiomaOfimatica[' + index + '].cLegIdOfArchivo', '')
          formData.append('legIdiomaOfimatica[' + index + '].cLegIdOfValida', 'false')
          formData.append('legIdiomaOfimatica[' + index + '].cLegIdOfEstado', 'true')
        });

        this.lstserv.linvestigador.forEach((element: LegInvestigador, index) => {
          formData.append('legInvestigador[' + index + '].nLegInvCodigo', element.nLegInvCodigo.toString() ?? '0')
          formData.append('legInvestigador[' + index + '].nLegInvCentroRegistro', element.nLegInvCentroRegistro.toString() ?? '')
          formData.append('legInvestigador[' + index + '].nValorCentroRegistro', element.nValorCentroRegistro.toString() ?? '0')
          formData.append('legInvestigador[' + index + '].cLegInvNroRegistro', element.cLegInvNroRegistro.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegInvFechaInicio, "yyyy-MM-dd")
          formData.append('legInvestigador[' + index + '].dLegInvFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegInvFechaFin, "yyyy-MM-dd")
          formData.append('legInvestigador[' + index + '].dLegInvFechaFin', fecha)
          formData.append('legInvestigador[' + index + '].cFile', element.cLegInvArchivo)
          formData.append('legInvestigador[' + index + '].cLegInvArchivo', '')
          formData.append('legInvestigador[' + index + '].cLegInvValida', 'false')
          formData.append('legInvestigador[' + index + '].cLegInvEstado', 'true')
        });

        this.lstserv.lTesisAsesJur.forEach((element: LegTesisAseJur, index) => {
          formData.append('legTesisAseJur[' + index + '].nLegTesCodigo', element.nLegTesCodigo.toString() ?? '0')
          formData.append('legTesisAseJur[' + index + '].nLegTesTipo', element.nLegTesTipo.toString() ?? '0')
          formData.append('legTesisAseJur[' + index + '].nValorTipo', element.nValorTipo.toString() ?? '0')
          formData.append('legTesisAseJur[' + index + '].nLegTesNivel', element.nLegTesNivel.toString() ?? '0')
          formData.append('legTesisAseJur[' + index + '].nValorNivel', element.nValorNivel.toString() ?? '0')
          formData.append('legTesisAseJur[' + index + '].cLegTesNroResolucion', element.cLegTesNroResolucion.toString() ?? '')
          fecha = this.configserv.datepipe.transform(element.dLegTesFecha, "yyyy-MM-dd")
          formData.append('legTesisAseJur[' + index + '].dLegTesFecha', fecha)
          formData.append('legTesisAseJur[' + index + '].cFile', element.cLegTesArchivo)
          formData.append('legTesisAseJur[' + index + '].cLegTesArchivo', '')
          formData.append('legTesisAseJur[' + index + '].cLegTesValida', 'false')
          formData.append('legTesisAseJur[' + index + '].cLegTesEstado', 'true')
        });

        this.lstserv.lProduccionCiencia.forEach((element: LegProduccionCiencia, index) => {
          formData.append('legProduccionCiencia[' + index + '].nLegProdCodigo', element.nLegProdCodigo.toString() ?? '0')
          formData.append('legProduccionCiencia[' + index + '].cLegProdNroResolucion', element.cLegProdNroResolucion.toString() ?? '')
          formData.append('legProduccionCiencia[' + index + '].cLegProdTitulo', element.cLegProdTitulo.toString() ?? '')
          formData.append('legProduccionCiencia[' + index + '].nLegProdTipo', element.nLegProdTipo.toString() ?? '0')
          formData.append('legProduccionCiencia[' + index + '].nValorTipo', element.nValorTipo.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegProdFecha, "yyyy-MM-dd")
          formData.append('legProduccionCiencia[' + index + '].dLegProdFecha', fecha)
          formData.append('legProduccionCiencia[' + index + '].cFile', element.cLegProdArchivo)
          formData.append('legProduccionCiencia[' + index + '].cLegProdArchivo', '')
          formData.append('legProduccionCiencia[' + index + '].cLegProdValida', 'false')
          formData.append('legProduccionCiencia[' + index + '].cLegProdEstado', 'true')
        });

        this.lstserv.lParticipacion.forEach((element: LegParticipacionCongSem, index) => {
          formData.append('legParticipacionCongSem[' + index + '].nLegParCodigo', element.nLegParCodigo.toString() ?? '0')
          formData.append('legParticipacionCongSem[' + index + '].cLegParInstitucion', element.cLegParInstitucion.toString() ?? '')
          formData.append('legParticipacionCongSem[' + index + '].cLegParOtraInst', element.cLegParOtraInst.toString() ?? '')
          formData.append('legParticipacionCongSem[' + index + '].cLegParPais', element.cLegParPais.toString() ?? '')
          formData.append('legParticipacionCongSem[' + index + '].cLegParNombre', element.cLegParNombre.toString() ?? '')
          formData.append('legParticipacionCongSem[' + index + '].nLegParRol', element.nLegParRol.toString() ?? '0')
          formData.append('legParticipacionCongSem[' + index + '].nValorRol', element.nValorRol.toString() ?? '0')
          formData.append('legParticipacionCongSem[' + index + '].nLegParAmbito', element.nLegParAmbito.toString() ?? '0')
          formData.append('legParticipacionCongSem[' + index + '].nValorAmbito', element.nValorAmbito.toString() ?? '0')

          fecha = this.configserv.datepipe.transform(element.dLegParFecha, "yyyy-MM-dd")
          formData.append('legParticipacionCongSem[' + index + '].dLegParFecha', fecha)
          fechaFin = this.configserv.datepipe.transform(element.dLegParFechaFin, "yyyy-MM-dd")
          formData.append('legParticipacionCongSem[' + index + '].dLegParFechaFin', fechaFin)

          formData.append('legCapacitaciones[' + index + '].nLegParHoras', element.nLegParHoras.toString() ?? '0')

          formData.append('legParticipacionCongSem[' + index + '].cFile', element.cLegParArchivo)
          formData.append('legParticipacionCongSem[' + index + '].cLegParArchivo', '')
          formData.append('legParticipacionCongSem[' + index + '].cLegParValida', 'false')
          formData.append('legParticipacionCongSem[' + index + '].cLegParEstado', 'true')
        });

        this.lstserv.lAdminitrativaCarga.forEach((element: LegAdminitrativaCarga, index) => {
          formData.append('legAdminitrativaCarga[' + index + '].nLegAdmCodigo', element.nLegAdmCodigo.toString() ?? '0')
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmInstitucion', element.cLegAdmInstitucion.toString() ?? '')
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmOtraInst', element.cLegAdmOtraInst.toString() ?? '')
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmPais', element.cLegAdmPais.toString() ?? '')
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmDocumento', element.cLegAdmDocumento.toString() ?? '')
          formData.append('legAdminitrativaCarga[' + index + '].nLegAdmCargo', element.nLegAdmCargo.toString() ?? '0')
          formData.append('legAdminitrativaCarga[' + index + '].nClaseCargo', element.nClaseCargo.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegAdmFechaInicio, "yyyy-MM-dd")
          formData.append('legAdminitrativaCarga[' + index + '].dLegAdmFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegAdmFechaFin, "yyyy-MM-dd")
          formData.append('legAdminitrativaCarga[' + index + '].dLegAdmFechaFin', fecha)
          formData.append('legAdminitrativaCarga[' + index + '].cFile', element.cLegAdmArchivo)
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmArchivo', '')
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmValida', 'false')
          formData.append('legAdminitrativaCarga[' + index + '].cLegAdmEstado', 'true')
        });


        this.lstserv.lReconocimiento.forEach((element: LegReconocimiento, index) => {
          formData.append('legReconocimiento[' + index + '].nLegRecCodigo', element.nLegRecCodigo.toString() ?? '0')
          formData.append('legReconocimiento[' + index + '].cLegRecInstitucion', element.cLegRecInstitucion.toString() ?? '')
          formData.append('legReconocimiento[' + index + '].cLegRecOtraInst', element.cLegRecOtraInst.toString() ?? '')
          formData.append('legReconocimiento[' + index + '].cLegRecPais', element.cLegRecPais.toString() ?? '')
          formData.append('legReconocimiento[' + index + '].nLegRecDocumento', element.nLegRecDocumento.toString() ?? '0')
          formData.append('legReconocimiento[' + index + '].nValorDocumento', element.nValorDocumento.toString() ?? '0')
          formData.append('legReconocimiento[' + index + '].nLegRecTipo', element.nLegRecTipo.toString() ?? '0')
          formData.append('legReconocimiento[' + index + '].nValorTipo', element.nValorTipo.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegRecFecha, "yyyy-MM-dd")
          formData.append('legReconocimiento[' + index + '].dLegRecFecha', fecha)
          formData.append('legReconocimiento[' + index + '].cFile', element.cLegRecArchivo)
          formData.append('legReconocimiento[' + index + '].cLegRecArchivo', '')
          formData.append('legReconocimiento[' + index + '].cLegRecValida', 'false')
          formData.append('legReconocimiento[' + index + '].cLegRecEstado', 'true')
        });

        this.lstserv.lcapacitaciones.forEach((element: LegCapacitaciones, index) => {
          formData.append('legCapacitaciones[' + index + '].nLegCapCodigo', element.nLegCapCodigo.toString() ?? '0')
          formData.append('legCapacitaciones[' + index + '].cLegCapInstitucion', element.cLegCapInstitucion.toString() ?? '')
          formData.append('legCapacitaciones[' + index + '].cLegCapOtraInst', element.cLegCapOtraInst.toString() ?? '')
          formData.append('legCapacitaciones[' + index + '].cLegCapPais', element.cLegCapPais.toString() ?? '')
          formData.append('legCapacitaciones[' + index + '].cLegCapNombre', element.cLegCapNombre.toString() ?? '')
          formData.append('legCapacitaciones[' + index + '].nLegCapTipo', element.nLegCapTipo.toString() ?? '0')
          formData.append('legCapacitaciones[' + index + '].nValorTipo', element.nValorTipo.toString() ?? '0')
          formData.append('legCapacitaciones[' + index + '].nLegCapTipoEsp', element.nLegCapTipoEsp.toString() ?? '0')
          formData.append('legCapacitaciones[' + index + '].nValorTipoEsp', element.nValorTipoEsp.toString() ?? '0')
          formData.append('legCapacitaciones[' + index + '].nLegCapHoras', element.nLegCapHoras.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegCapFechaInicio, "yyyy-MM-dd")
          formData.append('legCapacitaciones[' + index + '].dLegCapFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegCapFechaFin, "yyyy-MM-dd")
          formData.append('legCapacitaciones[' + index + '].dLegCapFechaFin', fecha)
          formData.append('legCapacitaciones[' + index + '].cFile', element.cLegCapArchivo)
          formData.append('legCapacitaciones[' + index + '].cLegCapArchivo', '')
          formData.append('legCapacitaciones[' + index + '].cLegCapValida', 'false')
          formData.append('legCapacitaciones[' + index + '].cLegCapEstado', 'true')
        });

        this.lstserv.lProyeccionSoc.forEach((element: LegProyeccionSocial, index) => {
          formData.append('legProyeccionSocial[' + index + '].nLegProyCodigo', element.nLegProyCodigo.toString() ?? '0')
          formData.append('legProyeccionSocial[' + index + '].cLegProyInstitucion', element.cLegProyInstitucion.toString() ?? '')
          formData.append('legProyeccionSocial[' + index + '].cLegProyOtraInst', element.cLegProyOtraInst.toString() ?? '')
          formData.append('legProyeccionSocial[' + index + '].cLegProyPais', element.cLegProyPais.toString() ?? '')
          formData.append('legProyeccionSocial[' + index + '].cLegProyDescripcion', element.cLegProyDescripcion.toString() ?? '')
          formData.append('legProyeccionSocial[' + index + '].nLegProyTipo', element.nLegProyTipo.toString() ?? '0')
          formData.append('legProyeccionSocial[' + index + '].nValorTipo', element.nValorTipo.toString() ?? '0')
          fecha = this.configserv.datepipe.transform(element.dLegProyFechaInicio, "yyyy-MM-dd")
          formData.append('legProyeccionSocial[' + index + '].dLegProyFechaInicio', fecha)
          fecha = this.configserv.datepipe.transform(element.dLegProyFechaFin, "yyyy-MM-dd")
          formData.append('legProyeccionSocial[' + index + '].dLegProyFechaFin', fecha)
          formData.append('legProyeccionSocial[' + index + '].cFile', element.cLegProyArchivo)
          formData.append('legProyeccionSocial[' + index + '].cLegProyArchivo', '')
          formData.append('legProyeccionSocial[' + index + '].cLegProyValida', 'false')
          formData.append('legProyeccionSocial[' + index + '].cLegProyEstado', 'true')
        });



        this.regserv.registroFile('Registro de Legajos', 'legajo', formData).then((data) => {
          this.spinner.hide()
          if (this.segserv.usuarioreg.nRol == 1) {
            this.ctrlserv._banregister = false;
          }
        ...
        this.spinner.show()
        this.regserv.actualizarFile('Registro de Legajos', 'legajo/put/' + this.regLegDatosGenerales.nLegDatCodigo, formData).then((data) => {
          this.spinner.hide()
          if (this.segserv.usuarioreg.nRol == 1) {
            this.ctrlserv._banregister = false;
          }
  ...
  exportar_legajo($objaux: DatosUsuario) {
    ...
    window.open(
      environment.URLAPI + 'legajoexp-pdf/' + btoa($objaux.cPerCodigo) + '/' + btoa($objaux.nTipo.toString())
      ,
      "_blank"
    );
  }
  ver_datos($objaux: DatosUsuario) {
    if ($objaux.bLegajo) {
      this.lstserv.listado('legajo_percodigo', '/' + $objaux.cPerCodigo).then((data) => {
        data.vGradoAcad.cIntDescripcion = data.vGradoAcad.cIntDescripcion ?? ''
  ...
  editar_cv($objaux: DatosUsuario) {
    ...
    this.lstserv.listado('legajoaux/', $objaux.cPerCodigo).then((data) => {
      ...
      if (JSON.stringify(data) != '[]') {
        ...
        if (data.nLegDatCodigo == 0 || data.nLegDatCodigo == null) {
          Swal.fire("Control de Legajos", "Legajo no tiene datos generales, se cargaron datos de SEUSS, proceda a completar los datos y guardar.", "info")
        }
        this.regLegDatosGenerales = data;
        ...
        if ($obj.nClaseNacimiento != null && $obj.nClaseNacimiento != 0) {
          if ($obj.vNacimiento.cIntJerarquia.trim().length == 3) {
            let objnac: Interface[] = this.lstserv.lpaisNac.filter(x => x.nIntCodigo == $obj.vNacimiento.nIntCodigo)
            nacimiento = objnac.length > 0 ? objnac[0].nIntCodigo : 0
          } else {
            let filter = this.lstserv.lpaisNac.filter(x => x.cIntJerarquia.trim() == 'PER')
            nacimiento = filter[0].nIntCodigo
            this.ubigeoOnChange(filter[0], 0)

            filter = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim() == $obj.vNacimiento.cIntJerarquia.trim().substring(0, 2))
            dptonacimiento = filter[0].nIntCodigo
            this.ubigeoOnChange(filter[0], 4)

            filter = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim() == $obj.vNacimiento.cIntJerarquia.trim().substring(0, 4))
            provnacimiento = filter[0].nIntCodigo
            this.ubigeoOnChange(filter[0], 5)

            filter = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim() == $obj.vNacimiento.cIntJerarquia.trim().substring(0, 6))
            distnacimiento = filter[0].nIntCodigo
            this.ubigeoOnChange(filter[0], 6)
          }
        }
        if ($obj.nLetDatUbigeo != null && $obj.nLetDatUbigeo != 0) {
          let ubigeo = this.lstserv.lubigeo.filter(x => x.nIntCodigo == $obj.nLetDatUbigeo)[0]
          let filter = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim() == ubigeo.cIntJerarquia.trim().substring(0, 2))
          dptodomic = filter[0].nIntCodigo
          this.ubigeoOnChange(filter[0], 1)

          filter = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim() == ubigeo.cIntJerarquia.trim().substring(0, 4))
          provdomic = filter[0].nIntCodigo
          this.ubigeoOnChange(filter[0], 2)

          filter = this.lstserv.lubigeo.filter(x => x.cIntJerarquia.trim() == ubigeo.cIntJerarquia.trim().substring(0, 6))
        ...
        if ($obj.nValorCondicionColeg != null && $obj.nValorCondicionColeg != 0) {
          ...
          this.dateFEmi = $obj.dLegDatosFechaEmisionColeg == new Date(this.clmdserv.fechadef) ? new FormControl(new Date("")) : new FormControl(new Date($obj.dLegDatosFechaEmisionColeg))
          this.dateFExp = $obj.dLegDatosFechaExpiraColeg == new Date(this.clmdserv.fechadef) ? new FormControl(new Date("")) : new FormControl(new Date($obj.dLegDatosFechaExpiraColeg))
        }
        if ($obj.vGradoAcad != null) $obj.vGradoAcad.cIntDescripcion == $obj.vGradoAcad.cIntDescripcion ?? ""
        ...
        // console.log($obj.dLegDatFechaNacimiento)
        ...
        if ($obj.legGradoTitulo.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legGradoTitulo = $obj.legGradoTitulo.filter(x => x.cLegGraValida == false)
          }
          $obj.legGradoTitulo.forEach((item) => {
            ...
            item.cLegGraArchivo = item.cLegGraArchivo != "" ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegGraArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legDocenciaUniv.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legDocenciaUniv = $obj.legDocenciaUniv.filter(x => x.cLegDocValida == false)
          }
          $obj.legDocenciaUniv.forEach((item) => {
            ...
            item.cLegDocUniversidadNavigation.cPerNombre = item.cLegDocUniversidadNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            ...
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegDocArchivo = item.cLegDocArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegDocArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legCategoriaDocente.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legCategoriaDocente = $obj.legCategoriaDocente.filter(x => x.cLegCatValida == false)
          }
          $obj.legCategoriaDocente.forEach((item) => {
            ...
            item.cLegCatInstitucionNavigation.cPerNombre = item.cLegCatInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            item.vCategoria.cConDescripcion = item.vCategoria.cConDescripcion ?? ''
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegCatArchivo = item.cLegCatArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegCatArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legRegimenDedicacion.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legRegimenDedicacion = $obj.legRegimenDedicacion.filter(x => x.cLegRegValida == false)
          }
          $obj.legRegimenDedicacion.forEach((item) => {
            ...
            item.cLegCatInstitucionNavigation.cPerNombre = item.cLegCatInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            item.vDedicacion.cConDescripcion = item.vDedicacion.cConDescripcion ?? ''
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegRegArchivo = item.cLegRegArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegRegArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legProfesNoDocente.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legProfesNoDocente = $obj.legProfesNoDocente.filter(x => x.cLegProValida == false)
          }
          $obj.legProfesNoDocente.forEach((item) => {
            ...
            item.cLegProInstitucionNavigation.cPerNombre = item.cLegProInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            item.vCargo = (item.vCargo == null ? this.clmdserv.empty_constante() : item.vCargo)
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegProArchivo = item.cLegProArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegProArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legIdiomaOfimatica.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legIdiomaOfimatica = $obj.legIdiomaOfimatica.filter(x => x.cLegIdOfValida == false)
          }
          $obj.legIdiomaOfimatica.forEach((item) => {
            item.vCodigoDesc.cConDescripcion = item.vCodigoDesc.cConDescripcion ?? ""
            item.vNivel.cConDescripcion = item.vNivel.cConDescripcion ?? ''
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegIdOfArchivo = item.cLegIdOfArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegIdOfArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legInvestigador.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legInvestigador = $obj.legInvestigador.filter(x => x.cLegInvValida == false)
          }
          $obj.legInvestigador.forEach((item) => {
            item.vCentroRegistro.cIntDescripcion = item.vCentroRegistro.cIntDescripcion ?? ''
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegInvArchivo = item.cLegInvArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegInvArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legTesisAseJur.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legTesisAseJur = $obj.legTesisAseJur.filter(x => x.cLegTesValida == false)
          }
          $obj.legTesisAseJur.forEach((item) => {
            item.vNivel.cConDescripcion = item.vNivel.cConDescripcion ?? ''
            item.vTipo.cIntDescripcion = item.vTipo.cIntDescripcion ?? ''
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegTesArchivo = item.cLegTesArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegTesArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legProduccionCiencia.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legProduccionCiencia = $obj.legProduccionCiencia.filter(x => x.cLegProdValida == false)
          }
          $obj.legProduccionCiencia.forEach((item) => {
            item.vTipo.cIntDescripcion = item.vTipo.cIntDescripcion ?? ''
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegProdArchivo = item.cLegProdArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegProdArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legParticipacionCongSem.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legParticipacionCongSem = $obj.legParticipacionCongSem.filter(x => x.cLegParValida == false)
          }
          $obj.legParticipacionCongSem.forEach((item) => {
            ...
            item.cLegParInstitucionNavigation.cPerNombre = item.cLegParInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst

            if (item.vRol.cIntDescripcion == "EXPOSITOR") {
              item.vRol.cIntDescripcion = "PONENTE"
            }
            else {
            ...
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegParArchivo = item.cLegParArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegParArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legAdminitrativaCarga.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legAdminitrativaCarga = $obj.legAdminitrativaCarga.filter(x => x.cLegAdmValida == false)
          }
          $obj.legAdminitrativaCarga.forEach((item) => {
            ...
            item.cLegAdmInstitucionNavigation.cPerNombre = item.cLegAdmInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegAdmArchivo = item.cLegAdmArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegAdmArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legReconocimiento.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legReconocimiento = $obj.legReconocimiento.filter(x => x.cLegRecValida == false)
          }
          $obj.legReconocimiento.forEach((item) => {
            ...
            item.cLegRecInstitucionNavigation.cPerNombre = item.cLegRecInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegRecArchivo = item.cLegRecArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegRecArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legCapacitaciones.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legCapacitaciones = $obj.legCapacitaciones.filter(x => x.cLegCapValida == false)
          }
          $obj.legCapacitaciones.forEach((item) => {
            ...
            item.vInstitucion.cPerNombre = item.vInstitucion.cPerNombre ?? this.clmdserv.nombrenoinst
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegCapArchivo = item.cLegCapArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegCapArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legProyeccionSocial.length > 0) {
          if (this.configserv.nTareaValidar == this.bValidar) {
            $obj.legProyeccionSocial = $obj.legProyeccionSocial.filter(x => x.cLegProyValida == false)
          }
          $obj.legProyeccionSocial.forEach((item) => {
            ...
            item.vTipo.cConDescripcion = item.vTipo.cConDescripcion ?? ''
            item.cLegProyInstitucionNavigation.cPerNombre = item.cLegProyInstitucionNavigation.cPerNombre ?? this.clmdserv.nombrenoinst
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.cLegProyArchivo = item.cLegProyArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegProyArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legContratos.length > 0) {
          $obj.legContratos.forEach((item) => {
            ...
            item.cLegConArchivo = item.cLegConArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegConArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legResoluciones.length > 0) {
          $obj.legResoluciones.forEach((item) => {
            item.cUsuRegistro = item.cUsuRegistro.substr(item.cUsuRegistro.length - 3, 3)
            item.vResolucion.cConDescripcion = item.vResolucion.cConDescripcion ?? ''
            item.cLegResArchivo = item.cLegResArchivo ? (item.cUsuRegistro == 'pdf' ? environment.APIFILEPDF : environment.APIFILE) + item.cLegResArchivo : environment.CERTDEFAULT
        ...
        if ($obj.legDeclaracionJurada.length > 0) {
          this.lstserv.LegDeclaracionJurada.cLegDjanexo2 = environment.APIFILEPDF + $obj.legDeclaracionJurada[0].cLegDjanexo2 ?? ''
          this.lstserv.LegDeclaracionJurada.cLegDjanexo6 = environment.APIFILEPDF + $obj.legDeclaracionJurada[0].cLegDjanexo6 ?? ''
          this.lstserv.LegDeclaracionJurada.cLegDjanexo7 = environment.APIFILEPDF + $obj.legDeclaracionJurada[0].cLegDjanexo7 ?? ''
    ...
    if (this.configserv.nTareaValidar == this.bValidar) this.lstserv._banvalidar = true
  ...
  listar_DeclaracionJurada(ncodigo: number) {
    this.lstserv.listado('declaracionjurada_lst', '/' + ncodigo).then((data) => {

      let datos: LegDeclaracionJurada[] = data;
      if (datos.length > 0) {

        this.lstserv.LegDeclaracionJurada.cLegDjanexo2 = environment.APIFILEPDF + datos[0].cLegDjanexo2 ?? ''
        this.lstserv.LegDeclaracionJurada.cLegDjanexo6 = environment.APIFILEPDF + datos[0].cLegDjanexo6 ?? ''
        this.lstserv.LegDeclaracionJurada.cLegDjanexo7 = environment.APIFILEPDF + datos[0].cLegDjanexo7 ?? ''
        this.lstserv.LegDeclaracionJurada.cFileDjanexo2 = this.lstserv.LegDeclaracionJurada.cLegDjanexo2
        this.lstserv.LegDeclaracionJurada.cFileDjanexo6 = this.lstserv.LegDeclaracionJurada.cLegDjanexo6
        this.lstserv.LegDeclaracionJurada.cFileDjanexo7 = this.lstserv.LegDeclaracionJurada.cLegDjanexo7
        this.erranexo2 = true
